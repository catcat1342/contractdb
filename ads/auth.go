...
// func writeDicToTable(db *sql.DB, ind IndexInfo, flag int, dicItems []acc.DicItem, dicDigest bn.G1Affine) error {
// 	// parse parameters
// 	var err error
// 	authtable := ind.authTable()
// 	// if auth table exists, delete it, because this is the setup process
// 	if CheckTableExist(db, []string{authtable}) {
// 		_, err = db.Exec("DROP TABLE " + authtable)
// 		if err != nil {
// 			return err
// 		}
// 	}
...
// 	switch flag {
// 	case 0:
// 		// key, nxt, value (G1 for eq; G2 for range), fI (G1), w (G2)
// 		createTableQuery = "CREATE TABLE IF NOT EXISTS " + authtable + " (hkey BIGINT UNSIGNED, hnxt BIGINT UNSIGNED, valueX BINARY(32),valueY BINARY(32), digestX BINARY(32), digestY BINARY(32), witX0 BINARY(32), witX1 BINARY(32), witY0 BINARY(32), witY1 BINARY(32))"
// 	case 1:
// 		// key, nxt, value (G1 for eq; G2 for range), fI (G1), w (G2)
// 		createTableQuery = "CREATE TABLE IF NOT EXISTS " + authtable + " (hkey BIGINT UNSIGNED, hnxt BIGINT UNSIGNED, valueX0 BINARY(32),valueX1 BINARY(32), valueY0 BINARY(32), valueY1 BINARY(32), digestX BINARY(32), digestY BINARY(32), witX0 BINARY(32), witX1 BINARY(32), witY0 BINARY(32), witY1 BINARY(32))"
// 	default:
// 		log.Panicf("invalid flag")
...
// 	switch flag {
// 	case 0:
// 		stmt, err = tx.Prepare("INSERT INTO " + authtable + " VALUES (?,?,?,?,?,?,?,?,?,?)")
// 		if err != nil {
// 			return fmt.Errorf("failed to prepare statement: %v", err)
// 		}
// 		defer stmt.Close()
// 		for _, d := range dicItems {
// 			val := d.Value.(bn.G1Affine)
// 			_, err := stmt.Exec(d.Key.Uint64(), d.Nxt.Uint64(), acc.ToLong(val.X.Bytes()), acc.ToLong(val.Y.Bytes()), acc.ToLong(d.FI.X.Bytes()), acc.ToLong(d.FI.Y.Bytes()), acc.ToLong(d.W.X.A0.Bytes()), acc.ToLong(d.W.X.A1.Bytes()), acc.ToLong(d.W.Y.A0.Bytes()), acc.ToLong(d.W.Y.A1.Bytes()))
// 			if err != nil {
// 				return fmt.Errorf("failed to insert data: %v", err)
// 			}
// 		}
// 		// let hkey=INF, hnxt=null, value=null, fI=dicDigest
// 		_, err := stmt.Exec(acc.INF.Uint64(), nil, nil, nil, acc.ToLong(dicDigest.X.Bytes()), acc.ToLong(dicDigest.Y.Bytes()), nil, nil, nil, nil)
// 		if err != nil {
// 			return fmt.Errorf("failed to insert data: %v", err)
// 		}
// 	case 1:
...
// 		defer stmt.Close()
// 		for _, d := range dicItems {
// 			val := d.Value.(bn.G2Affine)
// 			_, err := stmt.Exec(d.Key.Uint64(), d.Nxt.Uint64(), acc.ToLong(val.X.A0.Bytes()), acc.ToLong(val.X.A1.Bytes()), acc.ToLong(val.Y.A0.Bytes()), acc.ToLong(val.Y.A1.Bytes()), acc.ToLong(d.FI.X.Bytes()), acc.ToLong(d.FI.Y.Bytes()), acc.ToLong(d.W.X.A0.Bytes()), acc.ToLong(d.W.X.A1.Bytes()), acc.ToLong(d.W.Y.A0.Bytes()), acc.ToLong(d.W.Y.A1.Bytes()))
// 			if err != nil {
// 				return fmt.Errorf("failed to insert data: %v", err)
// 			}
// 		}
// 		// let hkey=INF, hnxt=null, value=null, fI=dicDigest
// 		_, err := stmt.Exec(acc.INF.Uint64(), nil, nil, nil, nil, nil, acc.ToLong(dicDigest.X.Bytes()), acc.ToLong(dicDigest.Y.Bytes()), nil, nil, nil, nil)
// 		if err != nil {
// 			return fmt.Errorf("failed to insert data: %v", err)
// 		}
// 	default:
...
// 	}
// 	log.Printf("write dic auth to table %v\n", authtable)
// 	return nil
// }
